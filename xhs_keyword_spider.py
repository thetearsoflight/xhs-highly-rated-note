...
class XHSKeywordSpider:
    ...
    def crawl_keyword_notes(self, keyword, target_count=50):
        ...
        for scroll_idx in range(max_scrolls):
            ...
            # 模拟人类滚动行为 - 随机滚动距离和速度
            scroll_distance = random.randint(500, 800)  # 增加滚动距离
            scroll_pause = random.uniform(0.8, 1.5)  # 减少暂停时间，加快速度
            ...
            for retry in range(max_retries):
                try:
                    ...
                    if retry < max_retries - 1:
                        print(f"滚动失败，正在重试 ({retry + 1}/{max_retries})...")
                        time.sleep(2)
                    else:
                        print(f"滚动失败: {e}")
                        raise
    ...
    def save_to_json(self):
        ...
        filename = os.path.join(data_dir, f"{self.keyword}_notes.json" if self.keyword else 'xhs_notes.json')
        ...
        for note in filtered_notes:
            output_data.append({
                ...
                '点赞数_数值': self.parse_likes(note.get('likes', '0')),
...
def main():
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='小红书关键词搜索爬虫')
    parser.add_argument('-k', '--keyword', type=str, help='搜索关键词')
    parser.add_argument('-n', '--num', type=int, default=50, help='需要采集的达标笔记数量(点赞>90)，默认50篇')
